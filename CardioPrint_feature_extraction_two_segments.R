if (1) {
  ...
  fs = 2000 # acquisition sampling frequency
  
  data <- read.csv("timestamps_without_neutral.csv")
  Examinee <- data$ID
  
  for (examinee in data$ID)
  {
    sequence <- 1 
    ...
    xinterval2 <- c(Rpeak[5]/fs - 0.5, Rpeak[5]/fs + 0.5)
    yinterval <- c(-1,1)
    ...
    while (sequence < 3) {
      ...
      while (ii < 5) {
        ...
        BX_int <- (X[ii] - B[ii])*1000/fs # vreme izmedju otvaranja i zatvaranja aortnog zalistka
        ...
        if(file.exists('/Fetureset/Features_without_neutral.txt')) {
          
          features2 <- read.table('/Fetureset/Features_without_neutral.txt', sep = '\t', dec = ",")
          rownames(features) <- as.character(nrow(features2)+1)
          features <- rbind(features2,features)

        }

        write.table(features,'/Fetureset/Features_without_neutral.txt', sep = '\t', dec = ",")
        library(writexl)
        write.csv(x = features, file = "/Fetureset/Features_without_neutral.csv", row.names = FALSE)   
